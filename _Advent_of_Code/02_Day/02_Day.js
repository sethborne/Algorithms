function checksum(inputArray){
    ...
    for(let i = 0; i < inputArray.length; i += 1){
        console.log("here");
        var min = inputArray[i][0];
        var max = inputArray[i][0];
        for(let j = 0; j < inputArray[i].length; j += 1){
            ...
            console.log(`i: ${i} | j: ${j} | max: ${max} | min: ${min}`);
        }
    ...
    let sum = 0;
    for(let s = 0; s < differenceArray.length; s += 1){
        if(differenceArray[s]){
            sum += differenceArray[s];
        }
    }
    console.log(sum);
    return sum;
